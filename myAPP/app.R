              ...
              fluidRow(
                box(title = "Key Findings", width = 12,
                    tags$ul(
                      ...
                      tags$li("The distribution of body mass shows a central tendency, with Gentoo penguins tending to have a higher average body mass compared to Adélie and Chinstrap penguins."),
                      tags$li("Flipper length distribution shows clustering, with Gentoo penguins having the longest flippers, followed by Chinstrap and Adélie."),
                      tags$li("Culmen length distribution is multimodal, with Gentoo having the longest, and Adélie the shortest."),
                      tags$li("Culmen depth is slightly skewed, with Gentoo having the shallowest depth and Chinstrap and Adélie having nearly identical average depths."),
                      tags$li("A strong positive correlation exists between body mass and flipper length, consistent across species."),
                      tags$li("Simpson’s Paradox is observed in the relationship between Culmen Depth and Flipper Length, with positive correlations emerging when grouped by species and sex.")
                    )
                ),
                
                # Adding gap between results and references section
                tags$br(),
      ...
      tabItem(tabName = "about",
              fluidRow(
                box(title = "About Me", width = 12,
                    p("I am Trishita Patra, a first-year MSc Data Science student at Chennai Mathematical Institute. This app was developed as part of the Visualization Course taught by Dr. Sourish Das."),
                    p("In this project, I applied data visualization techniques to uncover interesting insights from the Palmer Penguin dataset, with the aim of exploring differences between species and sex, and correlations among these physical
measurement"),
                    p("Feel free to contact me at: trishitap.mds2024@cmi.ac.in")
                )
              )
      )
    )
...
server <- function(input, output, session) {
  ...
  # Define the bar_plot_fill function (same as you've shared) | categorical variable.
  ...
  output$num_plot <- renderPlot({
    ...
    if (input$num_var == "Body Mass (g)") {
      ...
      # Custom visualization (your plot)
    ...
    gridExtra::grid.arrange(general_plot, custom_plot, ncol = 2)
  })
