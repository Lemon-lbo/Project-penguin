                ...
                tabPanel("Distribution of Categorical Variables", 
                         fluidRow(
                           box(
                             ...
                             # Add 4th option to the dropdown
                             selectInput("cat_var", "Select Categorical Variable:", 
                                         choices = c("Sex", "Species", "Island", "Multidimensional Overview of Penguin Demographics"), 
                                         selected = "Sex"),
                             
                             # Conditional output depending on selected variable
                             conditionalPanel(
                               condition = "input.cat_var != 'Multidimensional Overview of Penguin Demographics'",
                               plotOutput("cat_plot"),
                               h4("Interpretation:"),
                               textOutput("cat_interpretation")
                             ),
                             
                             conditionalPanel(
                               condition = "input.cat_var == 'Multidimensional Overview of Penguin Demographics'",
                               plotlyOutput("penguin3d", height = "600px"),
                               h4("Interpretation:"),
                               p("This interactive 3D plot shows the penguin counts by species and island, stacked by sex. Hover over each bar to see detailed counts. Note that within each species-island group, the observed numbers of male and female penguins are approximately equal. ")
                             )
                           )
                         )
                ),
              ...
              fluidRow(
                box(title = "Key Findings", width = 12,
                    tags$ul(
                      ...
                      tags$li("The sex ratio is also nearly identical for each species in each island, when viewed using a 3D bar plot structured along the Species × Island × Sex dimensions."),
              ...
              fluidRow(
                ...
                box(title = "Credits and Copyright", width = 12,
                    ...
                    tags$p(
                      ...
                      " framework in RStudio. Licensed under the ",
  ...
  output$penguin3d <- renderPlotly({
    ...
    fig <- fig %>%
      layout(
        ...
        title = list(
          text = "Visualizing Sex Parity Across Species and Islands",
          font = list(size = 14)
        ),
